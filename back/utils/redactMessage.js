...
async function redactMessage(msgid,io,room, index,message,file,type,size){
    try {
        let title=null
    ...
    io.in(room).emit('@redactMServer',{a:1,index:index,message:message,file:title})
    } catch (error) {
        io.in(room).emit('@redactMServer',{a:0,error})
        console.log(error)
    }
}
module.exports={redactMessage}